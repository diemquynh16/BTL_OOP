...
DonHang::DonHang() {};
DonHang::DonHang(string TenDonHang, string KH, string sdt, Date ngaymua, int SL, int doanhthu, int loinhuan, vector<XeMay*> motorbikes, vector<int> soluongxe) {
...
vector<XeMay*> DonHang::getMotorbikes() {
	return _motorbikes;
}
vector<int> DonHang::getsoluongmoiloai() {
	return _slxe;
}
...
DonHang* DonHang::parse(string buffer)
{
...
//QuanLyDonHang
void QuanLyDonHang::themDonhang(string name, DonHang order) {
	ordersName.push_back(name);
	orders.push_back(order);
}
void QuanLyDonHang::XemDanhSachCacDonHang() {
	gotoxy(3, 9);
...
void QuanLyDonHang::ChitietDonHang(string tenDonHang) {
	int check = 0;
	for (int i = 0; i < orders.size(); i++) {
...
void QuanLyDonHang::CapNhatDonHang(string tenDonHang) {
	int check = 0;
	for (int i = 0; i < orders.size(); i++) {
		if (orders[i].getTenDonHang() == tenDonHang) {
...
void QuanLyDonHang::XoaDonHang(string tenDonHang) {
	int check = 0;
	vector<XeMay*> temp;
...
void QuanLyDonHang::DoanhThu(int thang, int nam) {
	int doanhthu = 0;
	for (int i = 0; i < orders.size(); i++) {
...
void QuanLyDonHang::LoiNhuan(int thang, int nam) {
	int loinhuan = 0;
	for (int i = 0; i < orders.size(); i++) {
		if (orders[i].getNgayMua().getThang() == thang && orders[i].getNgayMua().getNam() == nam) {
...
QuanLyDonHang inputFileDonHang() {
	...
	ifstream reader("C:\\txt\\donhang.txt", ios::out);
...
int main() {
		...
		printMenu();
		ofstream writerBrand;
		writerBrand.open("C:\\txt\\brands.txt", ios::out);
		writerBrand <<"Danh sach:" << store.getBrand().size() << endl;;
		for (int i = 0; i < store.getBrand().size(); i++) 
		{
		...
		ofstream writer;
		writer.open("C:\\txt\\xemay.txt", ios::out);
		for (int i = 0; i < store.getBrand().size(); i++) {
		...
		ofstream writerDonHang;
		writerDonHang.open("C:\\txt\\donhang.txt", ios::out);
